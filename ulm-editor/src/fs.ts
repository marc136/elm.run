...
export const fs = new PreopenDirectory("/", [
    ['src', srcDir],
    ['tmp', new Directory([])],
    ['elm-home', new Directory([
        ['0.19.1', new Directory([
            ['packages', pkgDir]
        ])]
    ])],
...
export function addSourceFile(name: string, content: string) {
    if (name.includes('/')) throw new Error('Creating directories is not supported yet')
...
export async function writeFile(filepath: string, content: string | ArrayBuffer) {
...
export function writeFileInDir(dir: Directory, name: string, content: string | ArrayBuffer) {
...
export async function unpackInto(basedir: string, tar: TarFileItem<Uint8Array>[]) {
    for (const file of tar) {
        ...
        fullpath = (basedir === '/' ? '' : basedir) + '/' + fullpath
