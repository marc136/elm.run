...
repl :: IO TestTree
repl = do
  putStrLn "getting hardcoded artifacts"
  -- artifacts <- Ulm.ReadArtifacts.getHardcodedArtifacts
  putStrLn "got hardcoded artifacts"
  files <- Golden.findByExtension [".elm"] "."
  -- print files
  pure $ testGroup "elm repl golden test"
      [ Golden.goldenVsString
          ...
          (repl2 <$> Data.ByteString.Lazy.readFile aFile)
        | aFile <- files,
...
-- repl1 artifacts state byteString =
  ...
  -- Data.ByteString.Lazy.Char8.pack "horst"
